...
@objc(Dyson) class Dyson : CDVPlugin {
    ...
    @objc(getBlackoutConfigFromServer:)
    func getBlackoutConfigFromServer(command: CDVInvokedUrlCommand) {
        var pluginResult = CDVPluginResult(
            status: CDVCommandStatus_ERROR
        )

        UploadManager.sharedInstance.getBlackoutConfigFromServer()
        { (error, response) in
            ...
            if(error == nil){
                ...
                pluginResult = CDVPluginResult(
                    status: CDVCommandStatus_ERROR,
                    messageAs: "Data Send Failed! : \(error?.localizedDescription)"
                )
                Logger.sharedInstance.logError(error: "Data Send Failed! : \(error?.localizedDescription)")
            }
